"""VCR.py send-stubs for httpx2, adapted from vcr.stubs.httpx_stubs.

vcrpy patches httpx (0.x) clients only; these stubs do the same for httpx2
clients so the cassette-based tests keep working after the migration.
"""
...
_httpx2_signature = inspect.signature(httpx2.Client.request)

try:
    HTTPX2_REDIRECT_PARAM = _httpx2_signature.parameters["follow_redirects"]
except KeyError:
    HTTPX2_REDIRECT_PARAM = _httpx2_signature.parameters["allow_redirects"]
...
@patch("httpx2.Response.close", MagicMock())
@patch("httpx2.Response.read", MagicMock())
def _from_serialized_response(request, serialized_response, history=None):
    # Cassette format generated for HTTPX2 requests by older versions of
